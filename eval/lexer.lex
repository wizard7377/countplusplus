...
/*return yy::parser::make_NUMBER (std::stoi(yytext), loc);*/
%{ /* -*- C++ -*- */
# include <cerrno>
# include <climits>
# include <cstdlib>
...
%%
//yy::parser::make_NUMBER (std::stoi(s), loc);


yy::parser::symbol_type make_NUMBER (const std::string& s, const yy::parser::location_type& loc)
{
    return yy::parser::make_NUMBER (std::stold(s), loc);
}
...
driver::scan_begin ()
{
    ...
    FILE * pFile;
    pFile = tmpfile ();
    //if (pFile == NULL) { std::cout << "dun goffed up\n"; }
    std::cout << "file var is: " << file.c_str() << std::endl;
